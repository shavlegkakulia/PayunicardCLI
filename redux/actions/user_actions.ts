import { FETCH_USER_DETAILS, USER_LOADING, ACCOUNTS_LOADING, STATEMENTS_LOADING, FETCH_USER_ACCOUNTS, FETCH_ACCOUNT_STATEMENTS, FETCH_PAYMENT_ACCOUNT_STATEMENTS, FETCH_TRANSFER_ACCOUNT_STATEMENTS, FETCH_TOTAL_BALANCE, TOTAL_BALANCE_LOADING, FETCH_USER_PRODUCTS, USER_PRODUCTS_LOADING } from './../action_types/user_action_types';
import UserService, { IAccountBallances, IGetUserAccountsStatementResponse, IUnicard, IUserAccountsStatement, IUserAccountsStatementRequest } from './../../services/UserService';
import storage from './../../services/StorageService';
import { AUTH_USER_INFO } from '../../constants/defaults';
import { TYPE_UNICARD, UNICARD_PLUS, UNICARD_ULTRA, UPERA } from '../../constants/accountTypes';

export const FetchUserDetail = (remember?: boolean) => async (dispatch: any) => {
...
export const FetchUserProducts = () => (dispatch: any) => {
    ...
    UserService.GetUserProducts().subscribe({
        next: (Response) => {
            dispatch({ type: FETCH_USER_PRODUCTS, userProducts: Response.data?.data?.products });
        },
        error: () => dispatch({ type: USER_PRODUCTS_LOADING, isUserProductsLoading: false }),
        complete: () => dispatch({ type: USER_PRODUCTS_LOADING, isUserProductsLoading: false })
    })
}

export const FetchUserAccountStatements = (params: IUserAccountsStatementRequest, paging?: boolean, callback?:() => void) => (dispatch: any) => {
...
export const FetchUserTotalBalance = () => (dispatch: any) => {
    ...
    UserService.GetUserTotalBalance().subscribe({
        next: (Response) => {
            dispatch({ type: FETCH_TOTAL_BALANCE, userTotalBalance: Response.data.data });
        },
        error: () => dispatch({ type: TOTAL_BALANCE_LOADING, isTotalBalanceLoading: false }),
        complete: () => dispatch({ type: TOTAL_BALANCE_LOADING, isTotalBalanceLoading: false })
    })
}
    ...
    UserService.GetUserAccounts().subscribe({
        ...
        complete: () => {
            ...
            UserService.GetUnicards().subscribe({
                    ...
                    if (Response.data.ok) {
                        ...
                        userUnicards = tempUnicards?.map(uni => {
                            let uCard = {
                                ...
                                cards: [],
                                currencies: [{ key: "GEL", value: "₾", balance: (uni.amount || 0) / 10 }],
                ...
                complete: () => {
                    dispatch({ type: FETCH_USER_ACCOUNTS, userAccounts: { accountBallances: [...UserAccounts?.accountBallances, ...userUnicards] } });
