...
import { GetSwiftCategories, IGetSwiftResponseDataItem } from '../../../services/TransferServices';
...
const InternationalTransfer: React.FC<INavigationProps> = props => {
  ...
  const [{bankName, swiftCode}, setSwiftValues] = useState({bankName: '', swiftCode: ''});
...
const [codeErrorStyle, setCodeErrorStyle] = useState<StyleProp<ViewStyle>>(
  ...
  const setSelectedSwiftItem = (data?: IGetSwiftResponseDataItem) => {
    dispatch({type: TRANSFERS_ACTION_TYPES.SET_RECIVER_SWIFT, reciverSwift: data});
  }

  const setSelectedCountry = (data?: ICitizenshipCountry) => {
    dispatch({type: TRANSFERS_ACTION_TYPES.SET_RECIVER_COUNTRYCODE, reciverCountry: data});
  }

  const setReciverCity = (data?: string) => {
    dispatch({type: TRANSFERS_ACTION_TYPES.SET_RECIVER_CITY, reciverCity: data});
  }

  const setReciverAddress = (data?: string) => {
    dispatch({type: TRANSFERS_ACTION_TYPES.SET_RECIVER_ADDRESS, reciverAddress: data});
  }
  ...
  const GetPaymentDetails = () => {
    ...
    const data: IGetPaymentDetailsRequest = {
      ForOpClassCode: TransfersStore.selectedToCurrency?.key?.toLocaleUpperCase() === USD ? opClassCodes.internationalUsd : opClassCodes.internationalEur,
  ...
  useEffect(() => {
    if (userData.userAccounts && !userData.isAccountsLoading) {
      let uac = [...(userData.userAccounts?.filter(acc => acc.type !== TYPE_UNICARD && acc.type !== WALLET_TYPE) || [])];
      uac = uac.map(m => {
        m.currencies = [...(m.currencies || []).filter(s => s.key === USD || s.key === EUR)]
        return m;
    });
      setAccounts(uac);
    }
  ...
  const swiftCallDebouce = debounce((e: Function) => e(), 1000);
  ...
  useEffect(() => {
    swiftCallDebouce(() => {
      ...
      GetSwiftCategories(bankName, swiftCode).subscribe({
        ...
        }, complete: () => setSwiftDataFetching(false), 
        error: error => {setSwiftDataFetching(false)}
      });
    })
