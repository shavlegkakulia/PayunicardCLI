...
const TransactionsList: React.FC<IProps> = props => {
  ...
  const transactionItem = !props.unicards
    ? props.statements?.map((statement, index) => (
        <View style={styles.transactionsViewItem} key={`transactions${index}`}>
          <Image
            source={require('./../../../assets/images/mastercard_40x40.png')}
            style={styles.transactionsViewItemImage}
          />
          <TouchableOpacity
  ...
  return (
    <View style={containerStyle}>
      ...
      <ActionSheetCustom
        ...
        visible={transactionDetail != undefined}
        hasScroll={true}
        height={sheetHeight}
        onPress={() => setTransactionDetail(undefined)}>
