...
class PresentationService {
    GetCategories(ParentID: number = 0) {
        const promise = axios.get<ICategoryResponse>(`${envs.API_URL}GetCategories`, ParentID ? { params: { ParentID } } : {});
        return from(promise);
    }

    GetPaymentDetails(data: IGetPaymentDetailsRequest) {
        const promise = axios.get<IGetPaymentDetailsResponse>(`${envs.API_URL}GetPaymentDetails`, { params: data });
        return from(promise);
    }

    GetMerchantServices(data: IMerchantServicesForTemplateRequest) {
        const promise = axios.get<IMerchantServicesForTemplateResponseData>(`${envs.API_URL}GetMerchantServices`, { params: data });
        return from(promise);
    }

    SearchMerchants(value: string) {
        const promise = axios.get<ISearchMerchantServicesResponse>(`${envs.API_URL}SearchMerchantServices?Search=${value}`);
        return from(promise);
    }

    GetBatchPaymentDetails(data: IGetBatchPaymentDetailsRequets[]) {
        const promise = axios.post<IGetBatchPaymentDetailsRequetsData>(`${envs.API_URL}GetBatchPaymentDetails`, { paymentDetails: data }, { objectResponse: true });
        return from(promise);
    }

    GetCitizenshipCountries() {
        const promise = axios.get<ICountriesResponseData>(`${envs.API_URL}GetCitizenshipCountries`);
        return from(promise);
    }

    getCities() {
        const promise = axios.get<IIResponseOfGetCitiesResponse>(`${envs.API_URL}getCities`);
        return from(promise);
    }

    LogError(data: ILogErrorRequest) {
        const promise = axios.post<ILogErrorResponseData>(`${envs.API_URL}LogError`, data, { objectResponse: true });
        return from(promise);
    }

    getPackageTypes() {
        const promise = axios.get<IResponsePackageTypeListResponseData>(`${envs.API_URL}getPackageTypes`);
        return from(promise);
    }

    getCardTypes() {
        const promise = axios.get<IGetCardTypesResponseData>(`${envs.API_URL}getCardTypes`);
        return from(promise);
    }
}
